...
export default function AdminPricingPage() {
    ...
    const updateRate = async (id: string) => {
        ...
        setPricingReference((prev) =>
            prev.map((row) => (row.id === id ? { ...row, selected_rate: newRate } : row))
        );
    ...
    const updateJointRate = async (id: string) => {
        ...
        setJointPricing((prev) =>
            prev.map((row) => (row.id === id ? { ...row, unit_price: newRate } : row))
        );
        ...
        <div style={{ padding: 20 }}>
            ...
            <section style={{ marginTop: 30 }}>
                ...
                {pricingReference.map((row) => {
                    const currentValue =
                        editingRates[row.id] !== undefined ? editingRates[row.id] : row.selected_rate;
                    ...
                    return (
                        ...
                        >
                            ...
                            <button onClick={() => updateRate(row.id)} disabled={savingId === row.id}>
            ...
            <section style={{ marginTop: 30 }}>
                ...
                {jointPricing.map((row) => {
                    const currentValue =
                        editingJointRates[row.id] !== undefined ? editingJointRates[row.id] : row.unit_price;
                    ...
                    return (
                        ...
                        >
                            ...
                            <button onClick={() => updateJointRate(row.id)} disabled={savingId === row.id}>
                                {savingId === row.id ? "Saving..." : "Save"}
                            </button>
                        </div>
                    );
                ...
                {elementPricing.map((row) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <button onClick={() => updateElementRate(row.id)} disabled={savingId === row.id}>
                                {savingId === row.id ? "Saving..." : "Save"}
                            </button>
                        </div>
                    );
